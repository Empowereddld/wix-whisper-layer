    ...
    <header className="sticky top-0 z-50 h-[72px] bg-background border-b border-border">
      <div className="container h-full flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <Button size="default" className="rounded-md h-10 px-6 font-semibold">
            Login
          </Button>
        </nav>
      ...
      {/* Mobile nav */}
      {mobileOpen && (
        <nav className="md:hidden bg-background border-b border-border px-5 pb-6 pt-2 flex flex-col gap-4">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
