...
const HubDashboard = () => {
  ...
  useEffect(() => {
    if (profile && profile.welcome_dismissed === false) {
      setShowBanner(true);
    }
  }, [profile]);

  // Pre-filter to user's role on first load
  useEffect(() => {
    if (profile?.role) {
      setAudienceTab(profile.role);
    }
  ...
  const handleTypeFilter = (type: string) => {
    setActiveType(type);
    // Clear existing type filters and apply new one
    // We achieve this by clearing and toggling
  ...
  return (
    <HubLayout
      activeAudience={filters.audienceTab}
          ...
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
              <p className="text-sm sm:text-base">
                ...
                Here are your{" "}
                <span className="text-hub-lavender font-semibold">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-xs font-semibold text-stone-ui uppercase tracking-wide mr-1">
              Type:
            </span>
            {TYPE_PILLS.map((pill) => (
              <button
          ...
          !filters.audienceTab && (
            <div className="mb-10">
              <h2 className="text-xl font-bold text-midnight mb-4">
                Recommended for{" "}
                <span className="text-hub-lavender">
                  {roleLabel[profile?.role || "other"]}
                </span>
              </h2>
        ...
        <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
          <h2 className="text-xl font-bold text-midnight">
            {filters.audienceTab
              ? `${roleLabel[filters.audienceTab] ?? "All"} Resources`
              : "All Resources"}
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              variant="outline"
              size="sm"
              className="md:hidden border-thistle"
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap gap-2 mb-5">
            {filters.roles.map((r) => (
              <span
                key={r}
                className="flex items-center gap-1 px-3 py-1 rounded-full bg-thistle/60 text-midnight text-sm"
              >
                {r}
                <button
            ...
            <button
              ...
              className="text-xs text-stone-ui underline hover:text-midnight ml-1"
            >
              Clear all
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-80 max-w-[85vw] bg-card overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => setMobileFiltersOpen(false)}
                className="p-1 rounded-lg hover:bg-thistle/60"
              >
                <X className="h-5 w-5 text-midnight" />
              </button>
            </div>
            {/* Type pills in mobile drawer */}
              ...
              <div className="flex flex-wrap gap-2">
                {TYPE_PILLS.map((pill) => (
                  <button
                    key={pill.value}
                    onClick={() => {
                      handleTypeFilter(pill.value);
                      setMobileFiltersOpen(false);
                    }}
                    className={`px-3 py-1 rounded-full text-sm font-medium border transition-all ${
                      activeType === pill.value
                        ? "bg-midnight text-white border-midnight"
                        : "bg-card text-midnight border-thistle hover:border-hub-lavender"
                    }`}
                  >
                    {pill.label}
                  </button>
            ...
            {hasActiveFilters && (
              <Button
                ...
                className="w-full border-stone-ui"
              >
                Clear All Filters
              </Button>
            )}
